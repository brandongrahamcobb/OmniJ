 ...
 */
package com.brandongcobb.omnij.cogs;
...
import com.brandongcobb.omnij.objects.*;
import com.brandongcobb.omnij.registry.*;
import com.brandongcobb.omnij.service.*;
import com.brandongcobb.omnij.utils.handlers.*;
...
//
        ...
        } else if (!message.getContentDisplay().contains("@OmniJ")) {
            return;
        }
        ...
        CompletableFuture<String> contentFuture = (attachments != null && !attachments.isEmpty())
            ? mess.completeProcessAttachments(attachments).thenApply(list -> {
                multimodal[0] = true;
                return String.join("\n", list) + "\n" + message.getContentDisplay().replace("@OmniJ", "");
            })
            : CompletableFuture.completedFuture(message.getContentDisplay().replace("@OmniJ", ""));
        contentFuture
